...
var percentage = { "FIRSTQUARTILE": 25, "MIDPOINT": 50, "THIRDQUARTILE": 75, "COMPLETE": 95, "ENDED": 100 };
...
function getQuartileEvent(percentage$$1) {
  ...
  } else if (percentage$$1 >= percentage.ENDED && !eventsSent.includes(events.ENDED)) {
    return events.ENDED;
  }
  return null;
}
...
QUnit.test('the environment is sane', function (assert) {
  ...
  assert.strictEqual(typeof trackingEvents === 'undefined' ? 'undefined' : _typeof(trackingEvents), 'function', 'plugin is a function');
});

QUnit.module('videojs-tracking-events', {
...
QUnit.test('registers itself with video.js', function (assert) {
  ...
  assert.strictEqual(_typeof(Player.prototype.trackingEvents), 'function', 'videojs-tracking-events plugin was registered');

  this.player.trackingEvents();

  // Tick the clock forward enough to trigger the player to be "ready".
  this.clock.tick(1);

  assert.ok(this.player.hasClass('vjs-tracking-events'), 'the plugin adds a class to the player');
});
